...
// Add the 'disabled' class to the board on page load
board.classList.add('disabled');

// Add event listeners to the buttons
playerVsPlayerButton.addEventListener('click', () => {
    isAI = false; // Set the game mode to Player vs Player
    startGameMode();
});

playerVsAIButton.addEventListener('click', () => {
    isAI = true; // Set the game mode to Player vs AI
    startGameMode();
});
...
function handleClick(e){
    ...
    if (checkWin(currentClass)){
        ...
        if (isAI && circleTurn) {
            makeBestMove(); // If it's circle's turn and the game is against AI, call the makeBestMove function
        }
    }
}
...
function setBoardHoverClass(){
    ...
    board.classList.add(circleTurn ? CIRCLE_CLASS : X_CLASS); // Add the current class to the board
...
function endGame(draw){
    ...
    board.classList.add('disabled');
      ...
      // Make the game mode buttons visible again
    document.querySelector('.game-mode-buttons').classList.remove('hidden');
}
...
function makeBestMove() {
    ...
    if (bestMove) {
        bestMove.classList.add(AI_PLAYER);
    ...
    // The minimax function takes the current board state, depth, and a boolean indicating if it's the maximizing player's turn
