...
export default function App() {
  const [tasks, setTasks] = useState<string[]>([]); // Array of strings with initial value of an empty array 
  ...
  const addTask = () => {
    if (input.trim() !== "") { // Check if input is not empty or just spaces
      setTasks([...tasks, input.trim()]); // Add the new task to the tasks array
...
const headerStyle = "text-3xl font-extrabold mb-4 text-cyan-400 drop-shadow";
...
const buttonStyle = "bg-gradient-to-r from-cyan-500 to-purple-500 text-white px-5 py-2 rounded-r font-semibold hover:from-cyan-400 hover:to-purple-400 transition-colors";
const taskStyle = "flex justify-between items-center border-b border-gray-700 py-3 text-white";
const deleteButton = "text-cyan-300 hover:text-purple-400 transition-colors";
...
const pStyle = "text-gray-400 text-left mb-4";
  ...
  <div className={mainStyle}>
    <div className={mainContainer}>
      ...
      <p className={pStyle}>Made with Tailwind</p>
      <br />
