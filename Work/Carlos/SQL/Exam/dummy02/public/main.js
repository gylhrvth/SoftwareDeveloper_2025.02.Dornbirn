...
function createTodoDetailsDiv(todo) {
    ...
    // Create the base HTML for the details
    ...
    // Check if the todo has an image URL
    if (todo.todo_ImageURL) {
        const imageElement = document.createElement('img');
        imageElement.src = todo.todo_ImageURL; // Set the image source
        imageElement.alt = 'Task Image'; // Add an alt attribute
        imageElement.classList.add('todo-image'); // Optional: Add a class for styling
        todoDetailsDiv.appendChild(imageElement); // Append the image to the details div
    }
...
function handleEditFormSubmission(id) {
    ...
    const editTodoStatus = document.getElementById('editTodoStatus');
    const editTodoPriority = document.getElementById('editTodoPriority');
    const editPhoto = document.getElementById('editTodoPhoto');
    ...
    editTodoForm.onsubmit = async (e) => {
        ...
        console.log('Photo:', editPhoto.files[0]); // Log the selected file
        let imageUrl = null;
        try {
            if (editPhoto.files[0] !== undefined){
                const formData = new FormData();
                formData.append('uploaded_file', editPhoto.files[0]);
                let resultUpload = await fetch('/upload', {
                    method: 'POST',
                    body: formData
                });
                if (!resultUpload.ok) throw new Error('Failed to upload image');
                const dataUpload = await resultUpload.json();
                console.log('Image URL:', dataUpload.fileUri); // Log the uploaded image URL
                imageUrl = dataUpload.fileUri; // Use the uploaded image URL
            }

            // Call the refactored updateTodo function
            await updateTodo(id, updatedTask, updatedDescription, updatedStatus, updatedPriority, imageUrl);
...
async function updateTodo(id, updatedTask, updatedDescription, updatedStatus, updatedPriority, imgUrl) {
    try {
